...
void FxCircle::fast_clear_rect_(int x, int y, int w, int h) {
  ...
  static_assert(LV_COLOR_DEPTH >= 16,
                "FX_CIRCLE_FAST_CLEAR assumes true-color formats (>=16-bit).");
  ...
  lv_area_set(&a,
              ...
              (lv_coord_t)(x1 - 1), (lv_coord_t)(y1 - 1));
...
#else
    lv_canvas_draw_rect(canvas_, prev_cx_ - prev_r_, prev_cy_ - prev_r_,
                        prev_r_ * 2, prev_r_ * 2, &dsc_bg_);
#endif
  lv_canvas_draw_rect(canvas_, new_x, new_y, side, side, &dsc_fg_);
}

  prev_cx_ = cx; prev_cy_ = cy; prev_r_ = r;
}
