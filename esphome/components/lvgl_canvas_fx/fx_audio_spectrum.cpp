...
void FxAudioSpectrum::configure(int fft_n, int bars, float fs,
...
void FxAudioSpectrum::ensure_init_() {
  ...
  if (dsps_fft2r_init_fc32(twiddle_, fft_n_) != ESP_OK) {
    heap_caps_free(twiddle_); twiddle_ = nullptr;
    return;
  }
  ...
  if (data_mutex_ == nullptr) data_mutex_ = xSemaphoreCreateMutex();
...
void FxAudioSpectrum::on_data(const void* data, size_t bytes) {
  ...
  if ((int)fft_in_.size() != fft_n_ * 2) fft_in_.assign(fft_n_ * 2, 0.0f);
  ...
  // Bars
  if ((int)bar_.size() != num_bars_) bar_.assign(num_bars_, 0.0f);
  if ((int)peak_.size() != num_bars_) peak_.assign(num_bars_, 0.0f);
  ...
  if (last_rms_db_ < noise_gate_db_) {
    ...
    for (int i = 0; i < num_bars_; i++) {
      ...
      if (i == 0) norm *= bar0_scale_;
      if (i == 1) norm *= bar1_scale_;
      norm = std::clamp(norm, 0.0f, 1.0f);

      // Smooth + peak
      bar_[i] = bar_[i] * (1.0f - smooth_) + norm * smooth_;
      if (peak_[i] < bar_[i]) peak_[i] = bar_[i];
      else peak_[i] = std::max(0.0f, peak_[i] - peak_decay_);
    }
  }
...
void FxAudioSpectrum::step(float /*dt*/) {
  if (!canvas_ || area_.w <= 0 || area_.h <= 0) return;
  if (!dsc_init_) on_bind(canvas_);
  ...
  if (!ready || (int)bar_local.size() != num_bars_) return;
  ...
  for (int i = 0; i < num_bars_; i++) {
    ...
    if (y_peak < area_.y) y_peak = area_.y;
    dsc_peak_.bg_color = peak_color_;
